...
function highlight(){
    ...
    const searchLyrics = lyrics.split(' ')
    function findlyrics(e){
        function wordHighlight(word){
            return e.target.value !== '' && word.includes(e.target.value) === true ?
                    `<div class="highlight">${word}</div>` : word
        }
        const highlightLyrics = searchLyrics.map(wordHighlight)
        lyricsEl.innerHTML = highlightLyrics.join(' ')

    }

    searchEl.addEventListener('input', findlyrics)
